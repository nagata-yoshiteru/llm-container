#!/usr/bin/env python3
# Engram 用スリムツリー生成器。
# 出典: MiaAI-Lab/DeepSeek-v4.1-Flash-EXL3-2x-DGX-Sparks scripts/prepare_engram_src.py
# (AGPL-3.0 / MIT。機能をそのまま、この repo の fetch-model.sh から叩く形に合わせてある)
...
def prepare(src: Path, dst: Path) -> dict[str, object]:
    if src == dst:
        # 同一ディレクトリだと slim index がネイティブ index を踏んで再実行不能になる
        # (上流は常に別ディレクトリ想定)。
        raise SystemExit("--src and --dst must be different directories")
    ...
    if not keep:
        raise SystemExit(
            f"no {KEEP_SUFFIXES} keys in {index_path} — is --src the native V4.1 tree?"
        )
