...
local-inference-lab/vllm#597 (author: Jason, GitHub @original-el8; merged
2026-09-03 as ba233a68 on ``dev/jovian-judgement``):
https://github.com/local-inference-lab/vllm/pull/597

Runtime: ``b12x.comm.roce`` (RoCEnante) by @original-el8 (b12x#295, #315) and
Luke Alonso, @lukealonso (dual-rail striping aa90a277, DSV4 graph tuning
1a7e3ec2), https://github.com/local-inference-lab/b12x . This adapter targets
b12x commit b58f34ea (``API_VERSION == 1``, plain ``prepare()``). Newer b12x
(77351c13, 2026-09-14) requires prepared ``plan=`` objects and does NOT work
with this adapter.

Changes versus #597, for the dsv41-feat tree at e47aa780b:
- ``_parse_byte_size`` is inlined: this tree has no ``b12x_pcie_all_reduce.py``
  (the helper is copied from that file in the same fork).
- ``VLLM_ROCE_ALLGATHER_ENABLE`` (default 1) switches the all-gather route off
  on its own. It travels in the vote, so every rank agrees.

Contract with the runtime (from #597, unchanged):
...
def _parse_byte_size(value: str) -> int:
    """Parse "2MB", "84KB", "16MiB", "4096" into bytes.
...
class B12xRoceAllReduce:
    ...
    def __init__(
        ...
        if verdict is not None:
            ...
            return
        max_size, max_gather, gather_enabled = limits
        ...
        try:
            ...
            return
        self.disabled = False
        self._gather_enabled = bool(gather_enabled)
        if self.rank == 0:
            logger.info(
                ...
                max_size,
                max_gather,
                "on" if self._gather_enabled else "off",
            )

    def _local_capability(
        self,
    ) -> tuple[str | None, tuple[int, int, bool] | None]:
        ...
        Returns:
            ...
            and the parsed ``(max_size, max_gather, gather_enabled)`` limits
            (None on failure).
        """
        try:
            from b12x.comm import roce
        ...
        try:
            limits = (
                ...
                bool(envs.VLLM_ROCE_ALLGATHER_ENABLE),
            )
        except Exception as exc:  # noqa: BLE001 - reported through the vote
    ...
    def _exchange_vote(
        self, reason: str | None, limits: tuple[int, int, bool] | None
    ) -> str | None:
        ...
        """
        votes: list[tuple[str | None, tuple[int, int, bool] | None]] = [
            (None, None)
        ] * self.world_size
