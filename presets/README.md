...
> **⚠ DeepSeek-V4.1-Flash (EXL3) ブランチでは適用しないこと。**
> 以下は 0731 / Vision-Exp 時代のオーバーレイで、KV を `--kv-cache-dtype fp8` や
> `--block-size 256` で手で組み替える系譜。V4.1 は CSA2 KV の自動選択が前提で、
> これらを重ねると壊れる。コンテキストを動かすときは `.env` の
> `MAX_MODEL_LEN` / `VLLM_KV_ARGS` を直接編集すること
> (手順は README「コンテキストと調整ノブ」)。
...
> ⚠ **以下はすべて DeepSeek-V4-Flash-0731 / DSpark k=7 / bjk110 イメージでの実測。**
> 現在の既定は DeepSeek-V4-Flash-Vision-Exp (DSpark k=3 / 上流 vLLM 0.29 イメージ) な
> ので、そのままは当てはまらない。slots / max_concurrency も含めて計測し直すこと。
> KV 表の数値も同様。
